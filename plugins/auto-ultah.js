...
// Command handler
let handler = async (m, { conn, text, command, RafiTampilan }) => {
    const args = text.split('|');
    switch (command) {
        case 'addultah': {
            const [date, message] = args.map(v => v.trim());
            if (!date || !message) return RafiTampilan('Format: .addultah dd/mm/yyyy|pesan')

            addBirthday(date, message);
            RafiTampilan(`🎂 Ulang tahun berhasil ditambahkan:\n- Tanggal: ${date}\n- Pesan: ${message}`);
            break;
        }
        ...
        case 'delultah': {
            ...
            RafiTampilan(`🎂 Ulang tahun dengan pesan "${message}" berhasil dihapus.`);
            break;
        }

        case 'cekultah': {
            if (birthdayList.length === 0) {
                RafiTampilan('🎂 Tidak ada daftar ulang tahun.');
                return;
            }
        ...
        case 'autoultah': {
            ...
            if (status === 'on') {
                ...
                RafiTampilan('🎂 Auto ulang tahun diaktifkan. Pengingat akan dikirim otomatis setiap pukul 00:00.');
            } else if (status === 'off') {
                autoBirthdayEnabled = false;
                stopAutoBirthday();
                RafiTampilan('🎂 Auto ulang tahun dimatikan. Tidak ada pengingat ulang tahun otomatis.');
            } else {
                RafiTampilan('Format: .autoultah on/off');
            }
            break;
        }

        default:
            RafiTampilan('Perintah tidak valid.');
    }
};
