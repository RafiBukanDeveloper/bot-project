...
let handler = async (m, { conn, __dirname, args, usedPrefix, command, RafiTampilan }) => {
    try {
        let q = m.quoted ? m.quoted: m
        let mime = (q.msg || q).mimetype || ''
        if (!/audio/.test(mime)) return RafiTampilan(`Balas vn/audio yang ingin diubah dengan caption *${usedPrefix + command}*`)
        if (!args[0] || !args[1]) return RafiTampilan(`${usedPrefix + command} 00:00:30 00:00:30`)
