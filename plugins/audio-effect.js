...
let handler = async (m, { conn, args, __dirname, usedPrefix, command, RafiTampilan }) => {
    try {
        let q = m.quoted || m
        let mime = q.mimetype || ''
        let effects = {
        ...
        if (!set) return m.reply(`[ *Penggunaan Salah* ]\n${usedPrefix + command} Sambil Replay Audio Atau Voice Note`)
